...
class AppDataStore @Inject constructor(
    ...
    private object Keys {
        ...
        val CAMPUS_MODE = booleanPreferencesKey("campus_mode")
        val HIDE_PROFILE = booleanPreferencesKey("hide_profile")

        // Filters
        val FILTER_MAX_DISTANCE = floatPreferencesKey("filter_max_distance")
        val FILTER_CAMPUS_ONLY = booleanPreferencesKey("filter_campus_only")
    ...
    val privacySettings: Flow<PrivacySettings> = context.dataStore.data.map { prefs ->
        PrivacySettings(
            ...
            showDistance = prefs[Keys.SHOW_DISTANCE] ?: false,
            campusMode = prefs[Keys.CAMPUS_MODE] ?: true,
    ...
    suspend fun updatePrivacySettings(settings: PrivacySettings) {
        context.dataStore.edit { prefs ->
            ...
            prefs[Keys.SHOW_DISTANCE] = settings.showDistance
            prefs[Keys.CAMPUS_MODE] = settings.campusMode
    ...
    val filterSettings: Flow<FilterSettings> = context.dataStore.data.map { prefs ->
        FilterSettings(
            maxDistance = prefs[Keys.FILTER_MAX_DISTANCE] ?: 5f,
            campusOnly = prefs[Keys.FILTER_CAMPUS_ONLY] ?: true,
    ...
    suspend fun updateFilterSettings(settings: FilterSettings) {
        context.dataStore.edit { prefs ->
            prefs[Keys.FILTER_MAX_DISTANCE] = settings.maxDistance
            prefs[Keys.FILTER_CAMPUS_ONLY] = settings.campusOnly
